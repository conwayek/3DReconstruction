...
def main(direct,Nview=10,rangex=2000,rangey=2000):
    ...
    filenames = np.array(filenames,dtype=np.object)
    ...
    vert_dist = distance.distance(pt1,pt3).km
    horz_dist = distance.distance(pt1,pt2).km
    ...
    npix_x = nres_x/0.31
    npix_y = npix_x*(vert_dist/horz_dist)
    ...
    for i in range(nlat-1):
        ...
        for j in range(nlon-1):
            ...
            #if(np.sum(idf)>10):
            #    print(np.sum(np.isfinite(idf)))
            tile_count[i,j] = np.sum(idf)
            if(tile_count[i,j]>=Nview):
                #print(i,j,lat_upper,lon_left)
